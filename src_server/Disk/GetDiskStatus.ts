...
interface GetDiskStatusInterface {
    execute(diskPath: string, callback: (error: Error, stdout: string, stderr: string) => void): void;
}
...
class GetDiskStatus implements GetDiskStatusInterface {
    ...
    */
    public execute(diskPath: string, callback: (error: Error, stdout: string, stderr: string) => void): void {
        child_process.exec(`df --block-size=1024M ${ diskPath } | awk -v OFS=, 'NR == 2 {print $2, $3, $4;}'`, null, callback);
    }
}
